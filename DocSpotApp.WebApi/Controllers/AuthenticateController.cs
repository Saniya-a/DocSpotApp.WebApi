...
namespace DocSpotApp.WebApi.Controllers
{
    ...
    public class AuthenticateController : ControllerBase
    {
        ...
        public async Task<IActionResult> Register([FromBody] PatientVM model)
        {
        ...
        public async Task<IActionResult> RegisterAdmin([FromBody] AdminVM model)
        {
            var userExists = await _userManager.FindByNameAsync(model.Username);
            if (userExists != null)
                return StatusCode(StatusCodes.Status500InternalServerError, new Response { Status = "Error", Message = "User already exists!" });
        ...
        public async Task<IActionResult> RegisterDoctor([FromBody] DoctorVM model)
        {
            var userExists = await _userManager.FindByNameAsync(model.Username);
            if (userExists != null)
            ...
            var result = await _userManager.CreateAsync(user, model.Password);
            if (!result.Succeeded)
                return StatusCode(StatusCodes.Status500InternalServerError, new Response { Status = "Error", Message = "Doctor creation failed! Please check doctor details and try again." });
